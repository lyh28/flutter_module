...
class PersonBean {
  ...
  //左上角标签
  Tag tag;
  PersonBean(String imagestr,this.label, this.nickName,this.tag,this.roomId,this.userId){
  ...
  static List<PersonBean> getListFromJSON(Map map) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      PersonBean personBean = PersonBean(
          data[i]["imgPath"], title,name,tag.color==null?null:tag,data[i]["roomId"],data[i]["userId"]);
      list.add(personBean);
